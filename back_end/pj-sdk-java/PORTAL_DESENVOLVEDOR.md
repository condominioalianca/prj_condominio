...
```
Da mesma forma, ao emitir um boletoNovaAlianca, precisamos enviar um objeto do tipo Boleto, que por sua vez, contém outros objetos, como o pagador e o beneficiário final que são do tipo Pessoa:
```java
RespostaEmitirBoleto resposta = interSdk.cobranca().emitirBoleto(boletoNovaAlianca);
...
public class Boleto {
    ...
    /**
     * REQUERIDO
     * <p>Número de dias corridos após o vencimento para o cancelamento efetivo automático do boletoNovaAlianca. (de 0 até 60)/p>
     */
...
```java
Boleto boletoNovaAlianca = Boleto.builder()
        .seuNumero(seuNumero)
        ...
        .numDiasAgenda(0)               // Número de dias corridos após o vencimento para o cancelamento efetivo automático do boletoNovaAlianca. (de 0 até 60)
        .pagador(pagador)
        .build();
```
